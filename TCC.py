...
conf = float(input("Digite a confiabilidade em % conforme tabela 3: "))
while conf not in rel:
    print("Essa confiabilidade não está na tabela. Tente novamente.")
    conf = float(input("Digite a confiabilidade conforme tabela 3: "))
...
def Se(Sut, Acab, d, carga, conf):
    return Sel(Sut)*ka(Sut, Acab)*kb(d)*kc(carga)*ke(conf)
...
def fatordeseg(σal, σml, Se, Sut, σmax, Sy):
    if Goodman(σal, σml, Se, Sut) <= ηvm(σmax, Sy):
        ...
        if η < 1:
            print("Goodman indica a falha por fadiga do material.")
    else:
        ...
        if η < 1:
            print("Soderberg indica a falha por fadiga do material.")
...
while Decide != 1 and Decide != 2:
    Decide = int(input("Pressione 1 para calcular um fator de segurança com dimensões conhecidas ou 2 para achar uma dimensão pelo fator."))
