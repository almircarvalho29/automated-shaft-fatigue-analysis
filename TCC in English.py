...
def fatordeseg(σal, σml, Se, Sut, σmax, Sy):
    if Goodman(σal, σml, Se, Sut) <= ηvm(σmax, Sy):
        ...
        if η < 1:
            print("Caution! The safety factor is less than 1. A failure is indicated.")
    else:
...
while Decide != 1 and Decide != 2:
    Decide = int(input("Press 1 for safety factor calculation or 2 for nominal diameter calculation based on a given safety factor."))
